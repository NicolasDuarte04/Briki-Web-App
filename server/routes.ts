...
${posts.map(post => {
  ...
  const feedbackSchema = z.object({
    type: z.enum(['rating', 'suggestion', 'bug', 'feature']),
  ...
  app.post("/api/feedback", async (req: AuthenticatedRequest, res) => {
    try {
      ...
      const { type, rating, comment, page, userAgent, sessionId } = validation.data;
      
      // Insert feedback into database
      const result = await pool.query(`
        INSERT INTO feedback (type, rating, comment, page, user_agent, session_id, user_id)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING *
      `, [type, rating, comment, page, userAgent, sessionId, req.user?.id || null]);
  ...
  app.get("/api/feedback/analytics", async (req: AuthenticatedRequest, res) => {
    try {
      const { dateRange = '30d', type = 'all' } = req.query;
      ...
      const avgRatingResult = await pool.query(`
        ...
        WHERE rating IS NOT NULL ${dateFilter} ${typeFilter}
      `);
      const averageRating = parseFloat(avgRatingResult.rows[0].avg_rating) || 0;
      ...
      `);
      const feedbackByType = typeDistResult.rows.map(row => ({
        type: row.type,
        count: parseInt(row.count)
      }));

      // Feedback by page
      const pageDistResult = await pool.query(`
        SELECT page, COUNT(*) as count, AVG(rating) as avg_rating FROM feedback 
        WHERE 1=1 ${dateFilter} ${typeFilter}
        GROUP BY page ORDER BY count DESC LIMIT 10
      `);
      const feedbackByPage = pageDistResult.rows.map(row => ({
        page: row.page,
        count: parseInt(row.count),
      ...
      const recentResult = await pool.query(`
        SELECT id, type, rating, comment, page, created_at 
        FROM feedback 
        WHERE 1=1 ${dateFilter} ${typeFilter}
  ...
  // Get individual feedback details (admin only)
  app.get("/api/feedback/:id", isAuthenticated, async (req: AuthenticatedRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.patch("/api/feedback/:id/status", isAuthenticated, async (req: AuthenticatedRequest, res) => {
    try {
      const { id } = req.params;
  ...
  app.delete("/api/feedback/:id", isAuthenticated, async (req: AuthenticatedRequest, res) => {
    try {
      const { id } = req.params;
