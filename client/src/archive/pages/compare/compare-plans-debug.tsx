...
export default function ComparePlansDebug() {
  ...
  // Additional safety check to handle hydration issues
  const [storeInitialized, setStoreInitialized] = useState(false);
  
  // Initialize store and log selected plans on mount
  useEffect(() => {
    console.log("Current selected plans:", selectedPlans);
    // Mark store as initialized after first render
    ...
    <div className="container mx-auto px-4 py-8">
      {!storeInitialized ? (
        <div className="flex items-center justify-center h-40 bg-muted/30 rounded-lg">
          ...
          <p className="ml-3 text-muted-foreground">Initializing comparison tool...</p>
        </div>
      ) : (
        <>
                ...
                <Card key={plan.id} className="border border-border">
                  ...
                  <CardContent>
                    <div className="space-y-2">
                      ...
                      <p><strong>Price:</strong> {plan.price ? `$${plan.price}` : 'Not specified'}</p>
                    </div>
                  </CardContent>
                  <CardFooter>
                    <Button 
